...
SELECT DISTINCT S.`name`, S.`surname`, D.*, DEP.`name`, DEP.`id` FROM `students` AS S JOIN `degrees` AS D ON S.`degree_id` = D.`id` JOIN `departments` AS DEP ON D.`department_id` = DEP.`id`;
